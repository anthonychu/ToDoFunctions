...
namespace ToDoFunctions
{
    public static class ToDoOperations
    {
            ...
            TraceWriter log)
        {
            ...
            var includeCompleted = !req.GetQueryNameValuePairs()
                                       .Any(i => i.Key == "includecompleted" && i.Value == "false");

            var includeActive = !req.GetQueryNameValuePairs()
                                    .Any(i => i.Key == "includeactive" && i.Value == "false");

            var items = inTable
                .Where(p => (p.IsComplete == false || includeCompleted) && (p.IsComplete == true || includeActive))
                .ToList()
